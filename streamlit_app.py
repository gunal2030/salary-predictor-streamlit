...
import json
import numpy as np
import os # ADD THIS LINE AT THE VERY TOP OF YOUR SCRIPT, WITH OTHER IMPORTS
...
def load_model_files():
    try:
        # Construct the absolute path to the files using the script's directory
        # This is generally more robust for deployments
        script_dir = os.path.dirname(__file__) # Gets the directory where streamlit_app.py is
        model_path = os.path.join(script_dir, 'salary_model.pkl')
        scaler_path = os.path.join(script_dir, 'scaler.pkl')
        columns_path = os.path.join(script_dir, 'model_columns.json')
        ...
        st.error(f"Error: Model file not found. Details: {e}")
        st.error(f"Looked for files at: {os.path.dirname(__file__)}") # This will help debug the path
        st.error("Please ensure 'salary_model.pkl', 'scaler.pkl', and 'model_columns.json' are in the root of your GitHub repository alongside streamlit_app.py.")
        st.stop()
